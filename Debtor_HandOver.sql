use `promed`;
drop procedure if exists `Debtor_HandOver`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_HandOver`(in v_accountid bigint,
									in v_capturedby bigint,
									in vhandovertoid bigint,
...
/*
     ...
     date : 12 sep 2018
     purpose : to writeoff account for handover and flag account as handover
  ...
  set autocommit = 0;
   if ((@g_transAction_started = 0) or (@g_transAction_started is null)) then begin
    ...
    if (v_srcbalance > 0)  then begin
        ...
        set v_DESCRIPTION = concat('handover amount : ',(v_srcbalance));
        select ipkDebtorTransActionType,bVatCharge into v_debtortype,v_vatcharge from debtor_transAction_types where sTransactionTypeCode = 'handover';
        ...
        call Debtor_AccountToBadDebt(v_accountid, 0, 1, v_capturedby);
        call Debtor_WriteOff(v_accountid,v_DESCRIPTION,v_fAmount,v_capturedby,v_debtortype,vbatchNUMBER,v_addDate);
         
        call DebtorEventCapture(v_accountid, v_capturedby, 'handover', v_DESCRIPTION,null,v_addDate);	
		call Debtor_AddComment(concat('hand over - ',ifnull(v_DESCRIPTION,'')),v_accountid,v_capturedby,v_addDate);
		call Debtor_HandOverSave(0,v_accountid, v_capturedby,vhandoverdate,vhandovertoid,v_fAmount,v_fVatAmount,1,v_addDate);
    end; else begin
