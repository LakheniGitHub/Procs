use `promed`;
drop procedure if exists `Debtor_ReportingDailyBadDebtList`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailyBadDebtList`(in vday date, in vpracid bigint)
begin
