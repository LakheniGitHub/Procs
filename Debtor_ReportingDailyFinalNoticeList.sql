use `promed`;
drop procedure if exists `Debtor_ReportingDailyFinalNoticeList`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailyFinalNoticeList`(in vday date,in vpracid bigint)
begin
  /*
