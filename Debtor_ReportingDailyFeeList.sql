use `promed`;
drop procedure if exists `Debtor_ReportingDailyFeeList`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailyFeeList`(in vday date, in vpracid bigint)
begin
