use `promed`;
drop procedure if exists `Debtor_ReportingDailySmallBalanceList`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailySmallBalanceList`(in vday date, in vpracid bigint)
begin
