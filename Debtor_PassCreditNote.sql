use `promed`;
drop procedure if exists `Debtor_PassCreditNote`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_PassCreditNote`(in v_istatementid bigint,
						in  v_debtortransactiontype bigint(20) ,
...
begin
  /*
/*
  When a Fee Adj is done, a credit note is passed for the current invoice and a new invoice is created and all lines is re-added with the changed
  feeadj line in it. 
...
declare done boolean default 0;
 ...
 /*Build Current line list*/
   ...
   select ifkAccountID into  vaccid from debtor_statements where ipkStatementEntryID = v_istatementid;   
...
set v_snexttm  = '';
  set vvatexclwaarde = (vbal -  vvattot);
   ...
   select a.ifkSiteID,s.ifkPracticeInfoID,a.ifkBranchID into vsiteid,vpracid,vbranchid  from accounts a,sites s where a.ipkAccountID = v_accountid and a.ifkSiteID  = s.ipkSiteID;
    ...
    call debtorpracticefiscalinfofordate(vpracid,v_dlinedate,viLineFiscalYear,viLineFiscalQuarter,viLineFiscalMonth); 
	call debtorpracticefiscalinfofordate(vpracid,date(v_adddate),viRealFiscalYear,viRealFiscalQuarter,viRealFiscalMonth); 
...
insert into debtor_statement_audit (ipkStatementAuditID,ifkDebtorTransactionType,ifkaccountID,sBatchNumber,iBatchItemNumber,iTransactionLineNumber,dTransactionDate,
									...
									ifkDebtorAccountInvoiceID,sReceiptCode,ifkTaxType)
								values
								  (0,vcreditnotedebtype,v_accountid,'',0,v_ilinenumber,v_dlinedate,vsDescription,'',
									...
									videbtoraccountinvoiceid,'',v_taxtype);
	...
	call DebtorStatementLineEdit(v_istatementid ,  v_debtortransactiontype , v_taxtype , v_dlinedate ,  v_iquantity , v_sdescription , v_starrifcode ,v_snappicode , 
	                                               v_famount, v_fvatamount, v_fvatperc, v_sstructurecode, v_capturedby , v_batchnumber ,v_batchitemnumber, v_imethodofpayment ,
													 v_receiptcode , v_adddate );
