use `promed`;
drop procedure if exists `Debtor_ReportingDailyJournalListTotals`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailyJournalListTotals`(in vday date, in vpracid bigint)
begin
