use `promed`;
drop procedure if exists `Debtor_ReportingDailySmallBalanceListTotals`;

delimiter $$
use `promed`$$
create definer=`root`@`localhost` procedure `Debtor_ReportingDailySmallBalanceListTotals`(in vday date, in vpracid bigint)
begin
...
from debtor_statement_audit ds,accounts a,patients p,sites s,members m,medical_aid ma,debtor_transAction_types dtt,Branches b
 ...
 and dtt.ipkDebtorTransActionType = ds.ifkDebtorTransActionType
  and dtt.sTransactionTypeCode  in ('smallwoff')
